...
describe('buildMultiSegmentPath', () => {
  ...
  test('sets path edges to healthy by default', () => {
    const company = { id: 'c3', name: 'Company', lat: 10, lng: 10, node_type: 'company' };
    const suppliers = [
      {
        id: 's1',
        companyId: 's1',
    ...
    expect(result.edges.every((edge) => edge.status === 'healthy')).toBe(true);
  });
